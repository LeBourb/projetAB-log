...
if (!class_exists('class_new_user_approve')) {
    
    class class_new_user_approve {
            ...
            public static function instance() {
                    if ( !isset( self::$instance ) ) {
                            self::$instance = new class_new_user_approve();
            ...
            private function __construct() {
                    ...
                    remove_action('user_register','wp_new_user_notification');
                    ...
                    //( 'register_new_user', array( $this, 'request_admin_approval_email_2' ) , 400 );
            ...
            public function get_valid_statuses() {
                    return array( 'pending', 'approved', 'denied', 'confirm-email' );
            }
            ...
            public function authenticate_user( $userdata ) {
                    ...
                    switch ( $status ) {
                            ...
                            case 'denied':
                                    ...
                                    break;
                            case 'confirm-email':
                                    $denied_message = $this->default_authentication_message( 'confirm-email' );
                                    $message = new WP_Error( 'confirm_email_access', $denied_message );
                                    break;
                            case 'approved':
                                    $message = $userdata;
                                    break;
        ...
        public function get_count_of_user_statuses() {
            ...
            public function admin_approval_email( $user_login, $user_email ) {
                    ...
                    //WC()->mailer()->emails['WC_Email_Customer_Registration_New_User_Confirm_Email']->trigger($user_id, $activation_url);
            ...
            */
            ...
            public function add_user_status( $user_id ) {
                ...
                }else if ($_POST['account-type'] == 'professional'  || ( isset( $_REQUEST['action'] ) && 'createuser' == $_REQUEST['action'] )) {
                    $status = 'pending';
                }
                else {
                    return;
                ...
                if ($status == "confirm-email") {
                    ...
                    global $wpdb;
                    $wpdb->update( $wpdb->users, array( 'user_activation_key' => $code ), array( 'ID' => $user_id ) );
                    //$blogname = wp_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
                    $activation_url = add_query_arg( array( 'action' => 'confirm-email', 'key' => $code, 'user' => $user_id), wp_login_url() );
                    //$message = mail_new_user_confirm_email($user,$activation_url);
                    
                    //if(!has_action('woocommerce_registration_new_user_confirm_email_notification')) 
                    //    throw new Exception('No action for ' . $user_id . ' activation ' . $activation_url);
                    //do_action('woocommerce_registration_new_user_confirm_email_notification',$user_id, $activation_url);
                    WC()->mailer()->emails['WC_Email_Customer_Registration_New_User_Confirm_Email']->trigger($user_id, $activation_url);
                    //do_action('woocommerce_registration_new_user_admin_notification',$user_id, $activation_url);
                }else {
                    //do_action('woocommerce_registration_new_user_checking_pro_notification',$user_id);
                    //do_action('woocommerce_registration_new_user_checking_pro_admin_notification',$user_id);
                    // email to admin, to approve or deny the request.
                    $default_admin_url = admin_url( 'users.php?s&pw-status-query-submit=Filter&new_user_approve_filter=pending&paged=1' );
                    $admin_url = apply_filters( 'new_user_approve_admin_link', $default_admin_url );
                    WC()->mailer()->emails['WC_Email_Customer_Registration_Approval_Request_Pro']->trigger($user_id,$admin_url);


                    // email to user, informing that request is being processed.
                    WC()->mailer()->emails['WC_Email_Customer_Registration_New_User_Checking_Pro']->trigger($user_id);
...
function pw_new_user_approve() {
	return class_new_user_approve::instance();
}
